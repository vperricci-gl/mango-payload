...
export const landing_page_benefits_benefits_list = sqliteTable(
  'landing_page_benefits_benefits_list',
  {
    _order: integer('_order').notNull(),
    _parentID: integer('_parent_id').notNull(),
    id: text('id').primaryKey(),
  ...
  (columns) => ({
    _orderIdx: index('landing_page_benefits_benefits_list_order_idx').on(columns._order),
    _parentIDIdx: index('landing_page_benefits_benefits_list_parent_id_idx').on(columns._parentID),
    _parentIDFk: foreignKey({
      columns: [columns['_parentID']],
      foreignColumns: [landing_page.id],
      name: 'landing_page_benefits_benefits_list_parent_id_fk',
    }).onDelete('cascade'),
  }),
)

export const landing_page = sqliteTable(
  'landing_page',
  {
    id: integer('id').primaryKey(),
    title: text('title').notNull().default('Landing Page'),
    hero_titleText1: text('hero_title_text1').notNull().default('Transforme insights'),
    hero_titleText2: text('hero_title_text2').notNull().default('em métricas reais'),
    hero_subtitle: text('hero_subtitle').default(
      'Sua estratégia de OKR e acompanhamento de resultados em tempo real.',
    ),
    hero_ctaText: text('hero_cta_text').default('Comece sua transformação'),
    hero_ctaLink: text('hero_cta_link').default('#'),
    hero_heroImage: integer('hero_hero_image_id')
      .notNull()
      .references(() => media.id, {
        onDelete: 'set null',
      }),
    benefits_title: text('benefits_title').default('O coração do OKR bate dentro do Mangooh'),
    benefits_subtitle: text('benefits_subtitle').default(
      'Gerir metas deixou de ser tão complexo como imaginávamos na energia.',
    ),
    benefits_benefitImage: integer('benefits_benefit_image_id')
      .notNull()
      .references(() => media.id, {
        onDelete: 'set null',
      }),
    bottomSection_title: text('bottom_section_title').default(
      'Uma tecnologia projetada para o mundo real',
    ),
    bottomSection_subtitle: text('bottom_section_subtitle').default(
      'Simples, eficiente e que gera resultado.',
    ),
    bottomSection_backgroundImage: integer('bottom_section_background_image_id').references(
      () => media.id,
      {
        onDelete: 'set null',
      },
    ),
    bottomSection_ctaText: text('bottom_section_cta_text').default(
      'Comece agora sua transformação digital',
    ),
    bottomSection_ctaLink: text('bottom_section_cta_link').default('#'),
    updatedAt: text('updated_at')
      .notNull()
      .default(sql`(strftime('%Y-%m-%dT%H:%M:%fZ', 'now'))`),
    createdAt: text('created_at')
  ...
  (columns) => ({
    landing_page_hero_hero_hero_image_idx: index('landing_page_hero_hero_hero_image_idx').on(
      columns.hero_heroImage,
    ),
    landing_page_benefits_benefits_benefit_image_idx: index(
      'landing_page_benefits_benefits_benefit_image_idx',
    ).on(columns.benefits_benefitImage),
    landing_page_bottom_section_bottom_section_background_image_idx: index(
      'landing_page_bottom_section_bottom_section_background_image_idx',
    ).on(columns.bottomSection_backgroundImage),
    landing_page_updated_at_idx: index('landing_page_updated_at_idx').on(columns.updatedAt),
    landing_page_created_at_idx: index('landing_page_created_at_idx').on(columns.createdAt),
  }),
)
...
export const payload_locked_documents_rels = sqliteTable(
  'payload_locked_documents_rels',
  {
    ...
    categoriesID: integer('categories_id'),
    usersID: integer('users_id'),
    'landing-pageID': integer('landing_page_id'),
  ...
  (columns) => ({
    ...
    payload_locked_documents_rels_landing_page_id_idx: index(
      'payload_locked_documents_rels_landing_page_id_idx',
    ).on(columns['landing-pageID']),
    ...
    'landing-pageIdFk': foreignKey({
      columns: [columns['landing-pageID']],
      foreignColumns: [landing_page.id],
      name: 'payload_locked_documents_rels_landing_page_fk',
    }).onDelete('cascade'),
...
export const relations_landing_page_benefits_benefits_list = relations(
  landing_page_benefits_benefits_list,
  ({ one }) => ({
    _parentID: one(landing_page, {
      fields: [landing_page_benefits_benefits_list._parentID],
      references: [landing_page.id],
      relationName: 'benefits_benefitsList',
    }),
  }),
)
export const relations_landing_page = relations(landing_page, ({ one, many }) => ({
  hero_heroImage: one(media, {
    fields: [landing_page.hero_heroImage],
    references: [media.id],
    relationName: 'hero_heroImage',
  }),
  benefits_benefitImage: one(media, {
    fields: [landing_page.benefits_benefitImage],
    references: [media.id],
    relationName: 'benefits_benefitImage',
  }),
  benefits_benefitsList: many(landing_page_benefits_benefits_list, {
    relationName: 'benefits_benefitsList',
  }),
  bottomSection_backgroundImage: one(media, {
    fields: [landing_page.bottomSection_backgroundImage],
    references: [media.id],
    relationName: 'bottomSection_backgroundImage',
  }),
}))
...
export const relations_payload_locked_documents_rels = relations(
  ...
  ({ one }) => ({
    ...
    'landing-pageID': one(landing_page, {
      fields: [payload_locked_documents_rels['landing-pageID']],
      references: [landing_page.id],
      relationName: 'landing-page',
    }),
...
type DatabaseSchema = {
  ...
  users: typeof users
  landing_page_benefits_benefits_list: typeof landing_page_benefits_benefits_list
  ...
  relations_landing_page_benefits_benefits_list: typeof relations_landing_page_benefits_benefits_list
